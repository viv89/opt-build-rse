...
#        k_conv  = x[6]   
    ...
    def __init__(self, optimal_parameters, settings, build_data, hvac_data):
        ...
        self.data.pgamma        = 1.0
        self.data.ugamma        = 1.0
...
#        # Load updated variables in tseries object (for controller check)
    ...
    def objectiveCoefficients(self):
        ...
        Fmax_1 = self.data.pbuy*np.sum(abs(self.vardata.phi_hc_old))*np.mean(self.vardata.f_cop.values())
        Fmax_2 = num_max_hrs_discomfort = 2*self.data.hourly_steps*num_max_hrs_discomfort*deltaT_discomfort
        Fmax_3 = np.sum((0.1*abs(self.vardata.phi_hc_old[0:self.data.blocked_steps-1]) + 
                         0.1*abs(self.vardata.phi_hp_old[0:self.data.blocked_steps-1]))**2)
