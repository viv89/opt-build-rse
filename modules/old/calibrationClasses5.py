...
class calicop:
    ...
    def __init__(self, logs, hvac_data):
        ...
        self.history100 = self.history3[self.history3['rps']>59.0] # working points at full load (frequency > 59.0 Hz)
...
#        self.history0['Tst_mean'] = (self.history0['Tst_high'].values + self.history0['Tst_low'].values)/2                  \
    ...
    def update_qmax(self):
        ...
        if len(self.array.Xmax) > 5:
            self.calpars.coeff_qmax, self.info.score_qmax = self.heatPumpCoefficients(self.array.Xmax, self.array.ymax, 2)
        else:
            self.calpars.coeff_qmax, self.info.score_qmax = self.heatPumpCoefficients(self.array.Xmax, self.array.ymax, 1) 
...
class calihvac:
    ...
    def __init__(self, logs, hvac_data):
        ...
        if hvac_data['season'] == 'h':
            self.history = logs[logs['q_fc']>1.5][['m_fc','Tst_high','Tst_low','q_fc','Ti_A', 'Ti_B','Ti_C','Ti_D']]
        else:
            self.history = logs[logs['q_fc']<-1.5][['m_fc','Tst_high','Tst_low','q_fc','Ti_A', 'Ti_B','Ti_C','Ti_D']]    
    ...
    def update_fc(self, fdata):
        self.info.Tmin = min(fdata.Tset) - 2
        self.info.Tmax = max(fdata.Tset) + 2
        ...
        self.array.y_fc = 1000*self.history[['q_fc']].values
