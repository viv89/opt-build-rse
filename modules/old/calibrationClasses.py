...
class calipso:
    ...
    def __init__(self, building_props, df_ext, df_air, df_Qhc, df_irrad, cali_set, user_model):
...
#---------------------------------------------------------------------------------------------------
        ...
        # Periodic function with apparent user behaviour
        self.auxdata.phi_0 = user_model['avg_heat_gain']    # [W] average heat gain due to human activities
        self.auxdata.A     = user_model['amp_heat_gain']    # [W] maximum variation of heat gains (max-min)
        self.auxdata.T     = user_model['period']           # [hrs] period of heat gain variable signal
        self.auxdata.sfas  = user_model['time_shift']       # time-shift of heat gain variable signal
        ...
        self.calset.x_nom = [self.nompars.C_m, self.nompars.H_tr_em, self.nompars.H_tr_is, 
                             ...
                             self.auxdata.k_conv_nom, self.auxdata.fgc, 
                             self.auxdata.phi_0, self.auxdata.A, self.auxdata.T, self.auxdata.sfas]
...
#---------------------------------------------------------------------------------------------------       
        ...
        self.history['I_tot_gla'] = np.sum(np.multiply(df_irrad, self.auxdata.glazed_area),axis=1).values       
        self.history['I_tot_opa'] = np.sum(np.multiply(df_irrad, self.auxdata.opaque_area),axis=1).values
        ...
        self.history = self.history.tz_localize('Europe/Rome', ambiguous=True)          
  

#% CALIBRATION PROCESS ----------------------------------------------------------------------------   
        
    def update(self, df_air):
...
#        return Ti_weighted
    ...
    def runsim(self, x, hist, T_m0, tau):                   
        ...
        phi_0, amp, period, sfas = x[8], x[9], x[10], x[11]
        phi_int = phi_0 + amp*np.cos((2*np.pi/(period*3600))*time + sfas)
